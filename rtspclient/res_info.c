...
#define NEED_RES_OPCODES 1
#define NEED_RES_RESULTCODES 1
...
res_wks(int wks)
{
	...
	case 170: return("print-srv");
            default: (void) snprintf(retbuf, SIZE, "%d", wks); return(retbuf);
	}
}
...
res_proto(int proto)
{
	...
	case 17: return("udp");
            default: (void) snprintf(retbuf, SIZE, "%d", proto); return(retbuf);
	}
}
...
res_type(int type)
{
	...
	case T_UNSPEC:
		return("UNSPEC");
	default:
            (void)snprintf(retbuf, SIZE, "%d", type);
		return(retbuf);
	}
}
...
res_class(int class)
{
	...
	case C_ANY:		/* matches any class */
		return("ANY");
	default:
            (void)snprintf(retbuf, SIZE, "%d", class);
		return(retbuf);
	}
}
...
#define	PLURALIZE(x)	x, (x == 1) ? "" : "s"
	p = retbuf;
	if (days) {
            (void)snprintf(p, SIZE, "%d day%s", PLURALIZE(days));
		while (*++p);
	}
	if (hours) {
		if (days)
			*p++ = ' ';
		(void)snprintf(p, SIZE, "%d hour%s", PLURALIZE(hours));
		while (*++p);
	}
	if (mins) {
		if (days || hours)
			*p++ = ' ';
		(void)snprintf(p, SIZE, "%d min%s", PLURALIZE(mins));
		while (*++p);
	}
	if (secs || ! (days || hours || mins)) {
		if (days || hours || mins)
			*p++ = ' ';
		(void)snprintf(p, SIZE, "%d sec%s", PLURALIZE(secs));
	}
	return(retbuf);
}
