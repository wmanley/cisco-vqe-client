...
expand_cdname(char **cpp, char *msg)
{
	...
	if ((i = dn_expand((uint8_t *)msg, (uint8_t *)msg+512, 
                           (uint8_t *)*cpp, name, MAXDNAME)) < 0)
		return(NULL);
	if (name[0] == '\0' ) {
		name[0] = '.';
		name[1] = '\0';
...
parse_question(char **cpp, char *msg)
{
	...
	ptr->qtype = _getshort((uint8_t *)*cpp);
	*cpp += sizeof(u_short);
	ptr->qclass = _getshort((uint8_t *)*cpp);
	*cpp += sizeof(u_short);
...
parse_rr(char **cpp, char *msg)
{
	...
	ptr->type = _getshort((uint8_t *)*cpp);
	*cpp += sizeof(u_short);
	ptr->class = _getshort((uint8_t *)*cpp);
	*cpp += sizeof(u_short);
	ptr->ttl = _getlong((uint8_t *)*cpp);
	*cpp += sizeof(u_int);

	dlen = _getshort((uint8_t *)*cpp);
	ptr->dlen = dlen;
	*cpp += sizeof(u_short);
	...
	case T_SOA:				/* Start of Authority */
		...
		rd->soa.serial = _getlong((uint8_t *)*cpp);
		*cpp += sizeof(u_int);
		rd->soa.refresh = _getlong((uint8_t *)*cpp);
		*cpp += sizeof(u_int);
		rd->soa.retry = _getlong((uint8_t *)*cpp);
		*cpp += sizeof(u_int);
		rd->soa.expire = _getlong((uint8_t *)*cpp);
		*cpp += sizeof(u_int);
		rd->soa.minimum = _getlong((uint8_t *)*cpp);
		*cpp += sizeof(u_int);
		break;
	...
	case T_MX:				/* Mail Exchanger */
		rd->mx.preference = _getshort((uint8_t *)*cpp);
		*cpp += sizeof(u_short);
		rd->mx.exchange = expand_cdname(cpp, msg);
		break;

	case T_SRV:				/* Service location */
		rd->srv.priority = _getshort((uint8_t *)*cpp);
		*cpp += sizeof(u_short);
		rd->srv.weight = _getshort((uint8_t *)*cpp);
		*cpp += sizeof(u_short);
		rd->srv.port = _getshort((uint8_t *)*cpp);
		*cpp += sizeof(u_short);
	...
	case T_AFSDB:				/* AFS Server */
		rd->afsdb.subtype = _getshort((uint8_t *)*cpp);
		*cpp += sizeof(u_short);
	...
	case T_RT:				/* Route Through */
		rd->rt.preference = _getshort((uint8_t *)*cpp);
		*cpp += sizeof(u_short);
	...
	case T_UID:				/* User ID */
	case T_GID:				/* Group ID */
		rd->number = _getlong((uint8_t *)*cpp);
		*cpp += sizeof(u_int);
		break;
