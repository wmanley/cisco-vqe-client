 ...
 * uses RTP methods. This file holds all of the RTP SSM RSI SOURCE tests.
 *
...
rtp_ssm_rsi_source_t  *p_rtp_ssm_src = NULL; /* Used for creation and later 
                                       * deletion. */
...
/***********************************************************************
 *
 *     SSM RSI SOURCE SESSION SUITE TESTS START HERE
 *
 * SSM RSI SOURCE Session Suite tests the basic session functions (create 
 * and delete).
 *
 **********************************************************************/
...
void test_ssm_src_sess_delete(void)
{
    ...
    rtp_delete_session_ssm_rsi_source(&p_rtp_ssm_src, TRUE);
}

/***********************************************************************
 *
 *     SSM RSI SOURCE MEMBER FUNCTIONS SUITE TESTS START HERE
 *
 * SSM RSI SOURCE Member Functions Suite tests the basic member functions
 * using
...
//  memblocks_suite = alloc_blocks;
    ...
    if (TRUE != (rtp_create_session_ssm_rsi_source(&config, 
                                                   &p_rtp_ssm_src, 
                                                   TRUE))) {
        return (-1);
    }

    p_test_sess = (rtp_session_t *)p_rtp_ssm_src;
    return(0);
}
...
/***********************************************************************
 *
 *     SSM RSI SOURCE SENDER REPORT SUITE TESTS START HERE
 *
 * SSM RSI SOURCE Sender Report Suite tests the basic sender report 
 * functions using
