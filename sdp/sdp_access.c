/*
 *------------------------------------------------------------------
 * sdp_access.c  -- Routines for accessing data elements in SDP
 *                  structures, both setting and retrieving data.
 *
...
sdp_mca_t *sdp_find_media_level (sdp_t *sdp_p, u16 level)
{
    ...
    if ((level >= 1) && (level <= sdp_p->mca_count)) {
        for (i=1, mca_p = sdp_p->mca_p; 
             ((i < level) && (mca_p != NULL));
             mca_p = mca_p->next_p, i++); /* Empty for */
...
/* Function:    sdp_get_session_info
 * Description: Returns the session info parameter from the i= session
 *              info token line.  Value is returned as a const ptr and so 
 *              cannot be modified by the application.  
 * Parameters:  sdp_ptr     The SDP handle returned by sdp_init_description.
 * Returns:     Ptr to session info or NULL.
 */
const char *sdp_get_session_info (void *sdp_ptr)
{
    sdp_t *sdp_p = (sdp_t *)sdp_ptr;
...
tinybool sdp_bandwidth_valid (void *sdp_ptr, u16 level, u16 inst_num)
{
    ...
    if (bw_data_p != NULL) {
        if (((int) bw_data_p->bw_modifier < SDP_BW_MODIFIER_AS) || 
				       ...
				       u16 ttl, u16 num_of_addresses)
{
    ...
    if ((ttl > 0) && (ttl <= SDP_MAX_TTL_VALUE)) {
        conn_p->ttl = ttl;
    } else {
        if (sdp_p->debug_flag[SDP_DEBUG_ERRORS]) {
            SDP_ERROR("%s TTL value:: %u not valid for multicast address",
                      sdp_p->debug_str, ttl);
        }
        sdp_p->conf_p->num_invalid_param++;
        return (SDP_INVALID_PARAMETER);
    ...
    if (num_of_addresses > 0) {
        if (level != SDP_SESSION_LEVEL) {
            ...
            if (sdp_p->debug_flag[SDP_DEBUG_ERRORS]) {
                SDP_ERROR("%s Number of addresses for multicast can appear"
                         " only at media level", sdp_p->debug_str);
            }
            sdp_p->conf_p->num_invalid_param++;
            return (SDP_INVALID_PARAMETER);
        }
...
sdp_result_e sdp_insert_media_line (void *sdp_ptr, u16 level)
{
    ...
    if (level == 1) {
         ...
         */
        mca_p = sdp_find_media_level(sdp_p, level-1);
        if (mca_p == NULL) {
            SDP_FREE(new_mca_p);
            sdp_p->conf_p->num_invalid_param++;
            return (SDP_INVALID_PARAMETER);
...
void sdp_delete_media_line (void *sdp_ptr, u16 level)
{
    ...
    if (level == 1) {
        ...
        prev_mca_p = sdp_find_media_level(sdp_p, level-1);
        if (prev_mca_p == NULL) {
            sdp_p->conf_p->num_invalid_param++;
            return;
        }
 ...
 */
sdp_result_e sdp_add_new_bw_line (void *sdp_ptr, u16 level, u16 *inst_num)
{
    sdp_t               *sdp_p = (sdp_t *)sdp_ptr;
    sdp_bw_t            *bw_p;
    sdp_mca_t           *mca_p;
                         ...
                         sdp_bw_modifier_e bw_modifier, u32 bw_val)
{
    ...
    if (((int) bw_modifier < SDP_BW_MODIFIER_AS) || 
