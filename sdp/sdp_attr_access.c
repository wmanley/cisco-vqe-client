/*
 *------------------------------------------------------------------
 * sdp_attr_access.c  -- Routines for accessing SDP attributes.
 *
                               ...
                               sdp_attr_e attr_type, u16 *inst_num)
{
    ...
    *inst_num = 0;
    ...
    /* Some attributes are valid only under session level */ 
    } else {
        switch (attr_type) {
            case SDP_ATTR_CAT:
            case SDP_ATTR_GROUP:
	...
	for (i=0; i < SDP_NE_NUM_BMAP_WORDS; i++) {
        ...
        fmtp_p->rtcp_per_rcvr_bw = 0;
        
        break;
    case SDP_ATTR_RTPMAP:
    case SDP_ATTR_SPRTMAP:
    ...
    case SDP_ATTR_MPTIME:
        mptime = &(new_attr_p->attr.mptime);
        mptime->num_intervals = 0;
        break;
    case SDP_ATTR_GROUP:
        new_attr_p->attr.group_data.group_attr = SDP_GROUP_ATTR_UNSUPPORTED;
        new_attr_p->attr.group_data.num_group_id =0;
        for (i=0; i<SDP_MAX_GROUP_STREAM_ID; i++) {
            new_attr_p->attr.group_data.group_id_arr[i] = NULL;
        }
        break;
    default:
        break;
...
void sdp_copy_attr_fields (sdp_attr_t *src_attr_p, sdp_attr_t *dst_attr_p)
{
    ...
    case SDP_ATTR_FMTP:
        ...
        sstrncpy(dst_attr_p->attr.fmtp.cpcf, src_attr_p->attr.fmtp.cpcf,
                 SDP_MAX_STRING_LEN+1);

 
        for (i = 0; i < 5; i++) {
            dst_attr_p->attr.fmtp.video_pic_size[i].type =
                   src_attr_p->attr.fmtp.video_pic_size[i].type;
            dst_attr_p->attr.fmtp.video_pic_size[i].value =
                   src_attr_p->attr.fmtp.video_pic_size[i].value;
        } 
        ...
        dst_attr_p->attr.fmtp.rtcp_per_rcvr_bw = 
            src_attr_p->attr.fmtp.rtcp_per_rcvr_bw;
    ...
    case SDP_ATTR_GROUP:
        dst_attr_p->attr.group_data.group_attr =
            src_attr_p->attr.group_data.group_attr;
        dst_attr_p->attr.group_data.num_group_id =
             src_attr_p->attr.group_data.num_group_id;
        
        for (i=0; i < src_attr_p->attr.group_data.num_group_id; i++) {
            dst_attr_p->attr.group_data.group_id_arr[i] =
                strdup(src_attr_p->attr.group_data.group_id_arr[i]);
        }
        break;
    ...
    case SDP_ATTR_RTCP:
        dst_attr_p->attr.rtcp.port = src_attr_p->attr.rtcp.port;
        dst_attr_p->attr.rtcp.nettype = src_attr_p->attr.rtcp.nettype;
        dst_attr_p->attr.rtcp.addrtype = src_attr_p->attr.rtcp.addrtype;
        sstrncpy(dst_attr_p->attr.rtcp.conn_addr,
                 src_attr_p->attr.rtcp.conn_addr,
                 SDP_MAX_STRING_LEN+1);
        break;
	...
	if (src_attr_p->attr.srtp_context.session_parameters) {
	    dst_attr_p->attr.srtp_context.session_parameters =
	                strdup(src_attr_p->attr.srtp_context.session_parameters);
	}
                            ...
                            sdp_attr_e src_attr_type, u16 src_inst_num)
{
	...
	new_attr_p->attr.fmtp.hrd      = src_attr_p->attr.fmtp.hrd;
        ...
        sstrncpy(new_attr_p->attr.fmtp.cpcf, src_attr_p->attr.fmtp.cpcf,
                 SDP_MAX_STRING_LEN+1);
 
        for (i = 0; i < 5; i++) {
            new_attr_p->attr.fmtp.video_pic_size[i].type =
                   src_attr_p->attr.fmtp.video_pic_size[i].type;
            new_attr_p->attr.fmtp.video_pic_size[i].value =
                   src_attr_p->attr.fmtp.video_pic_size[i].value;
        } 
	...
	new_attr_p->attr.fmtp.fmtp_format   
        ...
        new_attr_p->attr.fmtp.rtcp_per_rcvr_bw = 
            src_attr_p->attr.fmtp.rtcp_per_rcvr_bw;
    ...
    case SDP_ATTR_GROUP:
        new_attr_p->attr.group_data.group_attr =
            src_attr_p->attr.group_data.group_attr;
        new_attr_p->attr.group_data.num_group_id =
            src_attr_p->attr.group_data.num_group_id;
        
        for (i=0; i < src_attr_p->attr.group_data.num_group_id; i++) {
            new_attr_p->attr.group_data.group_id_arr[i] =
                strdup(src_attr_p->attr.group_data.group_id_arr[i]);
        }
        break;

    case SDP_ATTR_RTCP:
        new_attr_p->attr.rtcp.port = src_attr_p->attr.rtcp.port;
        new_attr_p->attr.rtcp.nettype = src_attr_p->attr.rtcp.nettype;
        new_attr_p->attr.rtcp.addrtype = src_attr_p->attr.rtcp.addrtype;
        sstrncpy(new_attr_p->attr.rtcp.conn_addr,
                 src_attr_p->attr.rtcp.conn_addr,
                 SDP_MAX_STRING_LEN+1);
        break;
...
void sdp_free_attr (sdp_attr_t *attr_p) 
{
    ...
    } else if (attr_p->type == SDP_ATTR_GROUP) {
        for(i=0; i < attr_p->attr.group_data.num_group_id; i++) {
            SDP_FREE(attr_p->attr.group_data.group_id_arr[i]);
        }
    }
 ...
 *              bearer, called, connection_type, dialed, dialing,
 *              framing, and mid.
 * Parameters:  sdp_ptr     The SDP handle returned by sdp_init_description.
                                          ...
                                          u16 inst_num, u32 bool_parm)
{
    ...
    if (attr_p == NULL) {
        ...
        attr_p->attr.boolean_val = bool_parm;
        return (SDP_SUCCESS);
    }
}
                       ...
                       const char *string_parm)
{
    ...
    if (attr_p == NULL) {
        ...
        if (!sdp_validate_floating_point_string(string_parm)) {
            if (sdp_p->debug_flag[SDP_DEBUG_ERRORS]) {
                                         ...
                                         u8 cap_num)
{
    ...
    sdp_mca_t   *mca_p;
    sdp_attr_t  *attr_p;
    sdp_direction_e direction = SDP_DIRECTION_NOT_FOUND;
 ...
 * Returns:     Clockrate value.
 */
u16 sdp_attr_get_sprtmap_clockrate (void *sdp_ptr, u16 level,
                                   u8 cap_num, u16 inst_num)
{
    sdp_t       *sdp_p = (sdp_t *)sdp_ptr;
    sdp_attr_t  *attr_p;
...
/* Function:    sdp_get_next_video_pic_index
 * Description: Gets the next available free array index in the video picture size
 *              array.
 * Parameters:  fmtp_p:   The fmtp line structure                   
 * Returns:     The free array index, or -1 if there is no available index
 */
int sdp_get_next_video_pic_index (sdp_fmtp_t *fmtp_p) 
{
    int i = 0;

    for (i = 0; i < 5; i++) {
        if (fmtp_p->video_pic_size[i].type == SDP_VIDEO_PIC_INVALID_VALUE) {
            return i;
        }
    }
    return -1;
...
/* Function:    sdp_get_matching_video_pic_index
 * Description: Gets the array index matching the video picture size type
 * Parameters:  fmtp_p: The fmtp line structure                 
 *              type: The type of the video picture size, CIF, QCIF,
 *                    SQCIF, CIF4, CIF16
 * Returns:     The matching array index, or -1 if its not in the array
 */
int sdp_get_matching_video_pic_index (sdp_fmtp_t *fmtp_p,
                                sdp_video_picture_size type) 
{
    int i = 0;
 
    for (i = 0; i < 5; i++) {
        if (fmtp_p->video_pic_size[i].type == type) {
            return i;
        }
    }
    return -1;
...
/* Function:    sdp_set_video_pic_val
 * Description: Set the value of the particular picture size type in the 
 *              array of the fmtp line.
 * Parameters:  fmtp_p:   The fmtp line structure                   
 *              type:     The type of the video picture size, CIF, QCIF,
 *                        SQCIF, CIF4, CIF16
 * Returns:     SDP_SUCCESS  fmtp param was set successfully.
 *              SDP_FAILURE  Didn't find a free index to add this in.
 */
                             ...
                             sdp_video_picture_size type, u16 val) 
{
    ...
    if ((index = sdp_get_matching_video_pic_index(fmtp_p,
                                  type)) == -1) {
        index = sdp_get_next_video_pic_index(fmtp_p); 
    }
    if (index >= 0) {
	fmtp_p->video_pic_size[index].type = type;
	fmtp_p->video_pic_size[index].value = val;
	return (SDP_SUCCESS);
    } else {
        return (SDP_FAILURE);
    }
...
/* Function:    sdp_attr_set_fmtp_video_pic_size
 * Description: Sets the value of the different video picture size fmtp 
 *              parameter, CIF, QCIF, SQCIF, CIF4, CIF16
 * Parameters:  sdp_ptr     The SDP handle returned by sdp_init_description.
 *              level       The level to check for the attribute.  
 *              cap_num     The capability number associated with the
 *                          attribute if any.  If none, should be zero.
 *              inst_num    The attribute instance number to check.
 *              type        The type of video picture size
 *              cif         The value of this picture size
 * Returns:     SDP_SUCCESS            fmtp param was set successfully.
 *              SDP_INVALID_PARAMETER  Specified attribute is not defined.
 */
sdp_result_e sdp_attr_set_fmtp_video_pic_size (void *sdp_ptr, u16 level, 
                                     u8 cap_num, u16 inst_num, 
                                     sdp_video_picture_size type, u16 val)
{
    sdp_t       *sdp_p = (sdp_t *)sdp_ptr;
    ...
    return (sdp_set_video_pic_val(fmtp_p, type, val));
}
                                        ...
                                        u16 custom_x, u16 custom_y, u16 custom_mpi)
{
    ...
    if ((custom_x % 4) > 0) {
        if (sdp_p->debug_flag[SDP_DEBUG_ERRORS]) {
            SDP_ERROR("%s Invaild CUSTOM x value specified for "
                     "fmtp attribute at level %u instance %u : "
    ...
    if ((custom_y % 4) > 0) {
        if (sdp_p->debug_flag[SDP_DEBUG_ERRORS]) {
             SDP_ERROR("%s Invaild CUSTOM y value specified for "
                       " fmtp attribute at level %u instance %u : "
		       " Should be divisible by 4", 
		        sdp_p->debug_str, level, inst_num);
        }
	...
	if (sdp_p->debug_flag[SDP_DEBUG_ERRORS]) {
              SDP_ERROR("%s Invaild CUSTOM mpi value specified for "
                        "fmtp attribute at level %u instance %u : "
			 " Should be in range 1 and 32", 
		         sdp_p->debug_str, level, inst_num);
         }
         return (SDP_INVALID_PARAMETER);    
    }
                                     ...
                                     const char* cpcf)
{
    ...
    if (!sdp_validate_floating_point_string(cpcf)) {
        if (sdp_p->debug_flag[SDP_DEBUG_ERRORS]) {
            SDP_ERROR("%s is not a valid cpcf value.", cpcf);
        }
        sdp_p->conf_p->num_invalid_param++;
        return (SDP_INVALID_PARAMETER);
    }
    ...
    fmtp_p = &(attr_p->attr.fmtp);
    fmtp_p->fmtp_format = SDP_FMTP_CODEC_INFO;
    sstrncpy(fmtp_p->cpcf, cpcf, sizeof(fmtp_p->cpcf));
    return (SDP_SUCCESS);
}
                                    ...
                                    u32 bpp)
{
    ...
    if (((int) bpp < 0) || (bpp > SDP_MAX_BPP_HRD_VALUE)) {
        if (sdp_p->debug_flag[SDP_DEBUG_ERRORS]) {
            SDP_ERROR("%s Bpp value out of range for fmtp attribute, level %u "
	              "instance %u ", sdp_p->debug_str, level, inst_num);
        }
        return (SDP_INVALID_PARAMETER);
    }
...
sdp_result_e sdp_attr_set_fmtp_hrd (void *sdp_ptr, u16 level, 
                                    u8 cap_num, u16 inst_num, u32 hrd)
{
    sdp_t       *sdp_p = (sdp_t *)sdp_ptr;
    sdp_attr_t  *attr_p;
    sdp_fmtp_t  *fmtp_p;
    
    if (((int) hrd < 0) || (hrd > SDP_MAX_BPP_HRD_VALUE)) {
        if (sdp_p->debug_flag[SDP_DEBUG_ERRORS]) {
            SDP_ERROR("%s HRD value out of range for fmtp attribute, level %u "
	              "instance %u ", sdp_p->debug_str, level, inst_num);
        }
        return (SDP_INVALID_PARAMETER);
    }
...

{
    ...
    if ((annex_p_val_picture_resize < SDP_MIN_ANNEX_P_PIC_RESIZE_VALUE) ||
        (annex_p_val_picture_resize > SDP_MAX_ANNEX_P_PIC_RESIZE_VALUE)) {
        return (SDP_INVALID_PARAMETER);
    }

    if ((annex_p_val_warp < SDP_MIN_ANNEX_P_WARP_VALUE) ||
        (annex_p_val_warp > SDP_MAX_ANNEX_P_WARP_VALUE)) {
        return (SDP_INVALID_PARAMETER);
    }
...
/* Function:    sdp_attr_set_fmtp_apt
 * Description: Sets the value of the fmtp attribute apt parameter
 *              for the given attribute.
 * Parameters:  sdp_ptr     The SDP handle returned by sdp_init_description.
 *              level       The level to check for the attribute.  
 *              cap_num     The capability number associated with the
 *                          attribute if any.  If none, should be zero.
 *              inst_num    The attribute instance number to check.
 *              apt         Sets the apt value.
 * Returns:     SDP_SUCCESS            Attribute param was set successfully.
 *              SDP_INVALID_PARAMETER  Specified attribute is not defined.
 */
sdp_result_e sdp_attr_set_fmtp_apt  (void *sdp_ptr, u16 level, 
                                             u8 cap_num, u16 inst_num, 
                                             u16 apt)
{
    sdp_t       *sdp_p = (sdp_t *)sdp_ptr;
...
/* Function:    sdp_attr_set_fmtp_rtx_time
 * Description: Sets the value of the fmtp attribute rtx_time parameter
 *              for the given attribute.
 * Parameters:  sdp_ptr     The SDP handle returned by sdp_init_description.
 *              level       The level to check for the attribute.  
 *              cap_num     The capability number associated with the
 *                          attribute if any.  If none, should be zero.
 *              inst_num    The attribute instance number to check.
 *              rtx_time    Sets the rtx_time value.
 * Returns:     SDP_SUCCESS            Attribute param was set successfully.
 *              SDP_INVALID_PARAMETER  Specified attribute is not defined.
 */
sdp_result_e sdp_attr_set_fmtp_rtx_time  (void *sdp_ptr, u16 level, 
                                          u8 cap_num, u16 inst_num, 
                                          u32 rtx_time)
{
    sdp_t       *sdp_p = (sdp_t *)sdp_ptr;
...
/* Function:    sdp_attr_get_fmtp_rtcp_per_rcvr_bw
 * Description: Gets the value of the fmtp attribute-rtcp_per_rcvr_bw parameter
 * Parameters:  sdp_ptr     The SDP handle returned by sdp_init_description.
 *              level       The level to check for the attribute.  
 *              cap_num     The capability number associated with the
 *                          attribute if any.  If none, should be zero.
 *              inst_num    The attribute instance number to check.
 * Returns:     rtcp_per_rcvr_bw value.
 */
int32 sdp_attr_get_fmtp_rtcp_per_rcvr_bw (void *sdp_ptr, u16 level,
                                          u8 cap_num, u16 inst_num)
{
    ...
    if (attr_p == NULL) {
        ...
        return (attr_p->attr.fmtp.rtcp_per_rcvr_bw);
    }
}


/* Function:    sdp_attr_get_fmtp_video_pic_size
 * Description: Gets the value of the fmtp attribute type video picture size 
 *              parameter for a given Video codec. 
 * Parameters:  sdp_ptr     The SDP handle returned by sdp_init_description.
 *              level       The level to check for the attribute.  
 *              cap_num     The capability number associated with the
 *                          attribute if any.  If none, should be zero.
 *              inst_num    The attribute instance number to check.
 *              type        The type of video picture size, CIF, QCIF, SQCIF,
 *                          CIF4, CIF16
 *              priority    This field will get updated with the order this
 *                          particular picture size was received in the sdp.
 * Returns:     The value corresponding to the picture size type requested.
 */
int32 sdp_attr_get_fmtp_video_pic_size (void *sdp_ptr, u16 level,
                            u8 cap_num, u16 inst_num, 
                            sdp_video_picture_size type, 
                            int *priority)
{
	...
	*priority = -1;
        ...
        index = sdp_get_matching_video_pic_index(&(attr_p->attr.fmtp),
                                  type);
        if (index >= 0) {
            *priority = index;
            return (attr_p->attr.fmtp.video_pic_size[index].value);
        } else {
            *priority = -1;
            return (0);
        }
 ...
 * Returns:     CPCF value.
 */
const char* sdp_attr_get_fmtp_cpcf (void *sdp_ptr, u16 level,
                              u8 cap_num, u16 inst_num)
{
                                 ...
                                 u8 cap_num, u16 inst_num, u16 *val)
{
    ...
    if (attr_p == NULL) {
        ...
        if (attr_p->attr.fmtp.flag & SDP_PACKETIZATION_MODE_FLAG) {
 ...
 */
sdp_result_e sdp_attr_get_fmtp_profile_id (void *sdp_ptr, u16 level,
                                          u8 cap_num, u16 inst_num,
                                          const char** profile_level_id)
{
    ...
    if (attr_p == NULL) {
        ...
        if (attr_p->attr.fmtp.flag & SDP_PROFILE_LEVEL_ID_FLAG) {
            *profile_level_id = attr_p->attr.fmtp.profile_level_id;
            return (SDP_SUCCESS);
        } else {
            return (SDP_FAILURE);
        }
 ...
 */
sdp_result_e sdp_attr_get_fmtp_param_sets (void *sdp_ptr, u16 level,
                                          u8 cap_num, u16 inst_num,
                                          const char **param_sets)
{
    ...
    if (attr_p == NULL) {
        ...
        if (attr_p->attr.fmtp.flag & SDP_SPROP_PARAMETER_SETS_FLAG) {
            *param_sets = attr_p->attr.fmtp.parameter_sets;
            return (SDP_SUCCESS);
        } else {
            return (SDP_FAILURE);
        }
                                            ...
                                            u8 cap_num, u16 inst_num, u16* val)
{
    ...
    if (attr_p == NULL) {
        ...
        if (attr_p->attr.fmtp.flag & SDP_SPROP_INTERLEAVING_DEPTH_FLAG) {
                                      ...
                                      u32 *val)
{
    ...
    if (attr_p == NULL) {
        ...
        if (attr_p->attr.fmtp.flag & SDP_SPROP_MAX_DON_DIFF_FLAG) {
                                ...
                                u32 *val)
{
    ...
    if (attr_p == NULL) {
        ...
        if (attr_p->attr.fmtp.flag & SDP_MAX_MBPS_FLAG) {
                             ...
                             u8 cap_num, u16 inst_num, u32 *val)
{
    ...
    if (attr_p == NULL) {
        ...
        if (attr_p->attr.fmtp.flag & SDP_MAX_FS_FLAG) {
                                 ...
                                 u8 cap_num, u16 inst_num, u32 *val)
{
    ...
    if (attr_p == NULL) {
        ...
        if (attr_p->attr.fmtp.flag & SDP_MAX_CPB_FLAG) {
                               ...
                               u8 cap_num, u16 inst_num, u32 *val)
{
    ...
    if (attr_p == NULL) {
        ...
        if (attr_p->attr.fmtp.flag & SDP_MAX_DPB_FLAG) {
                             ...
                             u8 cap_num, u16 inst_num, u32* val)
{
    ...
    if (attr_p == NULL) {
        ...
        if (attr_p->attr.fmtp.flag & SDP_MAX_BR_FLAG) {
 ...
 */
sdp_result_e sdp_attr_fmtp_is_redundant_pic_cap (void *sdp_ptr, u16 level,
                                             u8 cap_num, u16 inst_num,
                                             tinybool *redundant_pic_cap)
{
    ...
    if (attr_p == NULL) {
        ...
        if (attr_p->attr.fmtp.flag & SDP_REDUNDANT_PIC_CAP_FLAG) {
            *redundant_pic_cap = attr_p->attr.fmtp.redundant_pic_cap;
            return (SDP_SUCCESS);
        } else {
            return (SDP_FAILURE);
        }
 ...
 */
sdp_result_e sdp_attr_fmtp_is_parameter_add (void *sdp_ptr, u16 level,
                                         u8 cap_num, u16 inst_num, 
                                         tinybool *parameter_add)
{
    ...
    if (attr_p == NULL) {
        ...
        if (attr_p->attr.fmtp.flag & SDP_PARAMETER_ADD_FLAG) {
            *parameter_add = attr_p->attr.fmtp.parameter_add;
            return (SDP_SUCCESS);
        } else {
            return (SDP_FAILURE);
        }
...
/* Function:    sdp_attr_get_fmtp_apt
 * Description: Gets the value of the fmtp attribute apt parameter
 *              for the given attribute.
 * Parameters:  sdp_ptr     The SDP handle returned by sdp_init_description.
 *              level       The level to check for the attribute.  
 *              cap_num     The capability number associated with the
 ...
 * Returns:     apt value.
 */
int32 sdp_attr_get_fmtp_apt (void *sdp_ptr, u16 level,
                                      u8 cap_num, u16 inst_num)
{
    ...
    if (attr_p == NULL) {
        ...
        return (attr_p->attr.fmtp.apt);
    }
}

/* Function:    sdp_attr_get_fmtp_rtx_time
 * Description: Gets the value of the fmtp attribute rtx_time parameter
 *              for the given attribute.
 * Parameters:  sdp_ptr     The SDP handle returned by sdp_init_description.
 *              level       The level to check for the attribute.  
 *              cap_num     The capability number associated with the
 *                          attribute if any.  If none, should be zero.
 *              inst_num    The attribute instance number to check.
 * Returns:     rtx_time value.
 */
int32 sdp_attr_get_fmtp_rtx_time (void *sdp_ptr, u16 level,
                                      u8 cap_num, u16 inst_num)
{
    ...
    if (attr_p == NULL) {
        ...
        return (attr_p->attr.fmtp.rtx_time);
    }
}


/* Function:    sdp_attr_get_rtcp_xr_loss_rle
 * Description: Gets the value of the rtcp-xr attribute pkt-loss-rle parameter
 *              for the given attribute.
 * Parameters:  sdp_ptr     The SDP handle returned by sdp_init_description.
 *              level       The level to check for the attribute.  
 *              cap_num     The capability number associated with the
 *                          attribute if any.  If none, should be zero.
 *              inst_num    The attribute instance number to check.
 * Returns:     pkt-loss-rle value.
 */
u16 sdp_attr_get_rtcp_xr_loss_rle (void *sdp_ptr, u16 level,
                                   u8 cap_num, u16 inst_num)
{
    ...
    attr_p = sdp_find_attr(sdp_p, level, cap_num, SDP_ATTR_RTCP_XR, 
                           inst_num);
    if (attr_p == NULL) {
        if (sdp_p->debug_flag[SDP_DEBUG_ERRORS]) {
            SDP_ERROR("%s rtcp-xr attribute, level %u instance %u "
                      "not found.", sdp_p->debug_str, level, inst_num);
        }
        sdp_p->conf_p->num_invalid_param++;
        return (SDP_INVALID_VALUE);
    } else {
        return (attr_p->attr.rtcp_xr.pkt_loss_rle_val);
    }
}


/* Function:    sdp_attr_get_rtcp_xr_per_loss_rle
 * Description: Gets the value of the rtcp-xr attribute per-loss-rle parameter
 *              for the given attribute.
 * Parameters:  sdp_ptr     The SDP handle returned by sdp_init_description.
 *              level       The level to check for the attribute.  
 *              cap_num     The capability number associated with the
 *                          attribute if any.  If none, should be zero.
 *              inst_num    The attribute instance number to check.
 * Returns:     post-error-repair-loss-rle value.
 */
u16 sdp_attr_get_rtcp_xr_per_loss_rle (void *sdp_ptr, u16 level,
                                       u8 cap_num, u16 inst_num)
{
    ...
    if (attr_p == NULL) {
        ...
        return (attr_p->attr.rtcp_xr.per_loss_rle_val);
    }
}


/* Function:    sdp_attr_get_rtcp_xr_stat_summary
 * Description: Gets the value of the rtcp-xr attribute stat-summary parameter
 *              for the given attribute.
 * Parameters:  sdp_ptr     The SDP handle returned by sdp_init_description.
 *              level       The level to check for the attribute.  
 *              cap_num     The capability number associated with the
 *                          attribute if any.  If none, should be zero.
 *              inst_num    The attribute instance number to check.
 * Returns:     stat-summary value.
 */
u32 sdp_attr_get_rtcp_xr_stat_summary (void *sdp_ptr, u16 level,
                                       u8 cap_num, u16 inst_num)
{
    ...
    if (attr_p == NULL) {
        ...
        return (attr_p->attr.rtcp_xr.stat_summary_val);
    }
}

/* Function:    sdp_attr_get_rtcp_rsize
 * Description: Gets the value of the rtcp-rsize attribute
 * Parameters:  sdp_ptr     The SDP handle returned by sdp_init_description.
 *              level       The level to check for the attribute.  
 *              cap_num     The capability number associated with the
 *                          attribute if any.  If none, should be zero.
 *              inst_num    The attribute instance number to check.
 * Returns:     rtcp-rsize value.
 */
tinybool sdp_attr_get_rtcp_rsize (void *sdp_ptr, u16 level,
                                  u8 cap_num, u16 inst_num)
{
    ...
    attr_p = sdp_find_attr(sdp_p, level, cap_num, SDP_ATTR_RTCP_RSIZE,
                           inst_num);
    if (attr_p == NULL) {
        if (sdp_p->debug_flag[SDP_DEBUG_ERRORS]) {
            SDP_ERROR("%s rtcp-rsize attribute, level %u instance %u "
                      "not found.", sdp_p->debug_str, level, inst_num);
        }
        sdp_p->conf_p->num_invalid_param++;
        return (SDP_INVALID_VALUE);
    } else {
        return (attr_p->attr.rtcp_rsize);
    }
}

/* Function:    sdp_attr_get_rtcp_xr_diagnostic_counters
 * Description: Gets the value of the rtcp-xr attribute vqe-diagnostic-counters 
 *              parameter for the given attribute.
 * Parameters:  sdp_ptr     The SDP handle returned by sdp_init_description.
 *              level       The level to check for the attribute.  
 *              cap_num     The capability number associated with the
 *                          attribute if any.  If none, should be zero.
 *              inst_num    The attribute instance number to check.
 * Returns:     vqe-diagnostic-counters value.
 */
tinybool sdp_attr_get_rtcp_xr_diagnostic_counters (void *sdp_ptr, u16 level,
                                                   u8 cap_num, u16 inst_num)
{
    ...
    if (attr_p == NULL) {
        ...
        return (attr_p->attr.rtcp_xr.diagnostic_counters_val);
    }
}

/* Function:    sdp_attr_get_rtcp_xr_multicast_acq
 * Description: Gets the value of the rtcp-xr attribute multicast-acq parameter
 *              for the given attribute.
 * Parameters:  sdp_ptr     The SDP handle returned by sdp_init_description.
 *              level       The level to check for the attribute.  
 *              cap_num     The capability number associated with the
 *                          attribute if any.  If none, should be zero.
 *              inst_num    The attribute instance number to check.
 * Returns:     multicast-acq value.
 */
tinybool sdp_attr_get_rtcp_xr_multicast_acq (void *sdp_ptr, u16 level,
                                             u8 cap_num, u16 inst_num)
{
    ...
    if (attr_p == NULL) {
        ...
        return (attr_p->attr.rtcp_xr.multicast_acq_val);
    }
}
                                          ...
                                          u16 m_line)
{
   ...
   for(p_type=1; p_type <=num_payload_types;p_type++){
       ...
       m_ptype = sdp_get_media_payload_type(sdp_p,
                                            m_line, p_type, &ind);
                                                 ...
                                                 u16 value, tinybool null_ind)
{
    ...
    if (attr_p == NULL) {
        ...
        attr_p->attr.silencesupp.fxnslevel = value;
                                     ...
                                     u8 cap_num, u16 inst_num)
{
    ...
    if (attr_p == NULL) {
        if (sdp_p->debug_flag[SDP_DEBUG_WARNINGS]) {
            SDP_WARN("%s Group (a= group line) attribute, level %u instance %u "
                     "not found.", sdp_p->debug_str, level, inst_num);
        }
        sdp_p->conf_p->num_invalid_param++;
        return (SDP_GROUP_ATTR_UNSUPPORTED);
       ...
       if (sdp_p->debug_flag[SDP_DEBUG_TRACE]) {
        ...
        return (attr_p->attr.group_data.group_attr);
    }
}
 ...
 * Parameters:  sdp_ptr     The SDP handle returned by sdp_init_description.
 *              level       SDP_SESSION_LEVEL
 *              group_attr  group attribute value ( LS/FID/FEC ).
                                 ...
                                 sdp_group_attr_e group_attr)
{
    ...
    if (attr_p == NULL) {
        ...
        attr_p->attr.group_data.group_attr = group_attr;
        return (SDP_SUCCESS);
    }
}
                          ...
                          u8 cap_num, u16 inst_num)
{
    ...
    return (attr_p->attr.group_data.num_group_id);
}


/* Function:    sdp_get_group_id
 * Description: Returns a particular id from the a=group:<>  line.  
 *                     a=group:<LS|FID> <id1> <id2> ...
 * Parameters:  sdp_ptr     The SDP handle returned by sdp_init_description.
 *              level       SDP_SESSION_LEVEL
 *              cap_num     The capability number associated with the
 *                          attribute if any.  If none, should be zero.
 ...
 * Returns:    Const string of the group id at the index specified or 
 *             NULL if an error
 */
                              ...
                              u8 cap_num, u16 inst_num, u16 id_num)
{
    ...
    if (attr_p == NULL) {
        ...
        if ((id_num < 1) || (id_num > attr_p->attr.group_data.num_group_id)) {
            if (sdp_p->debug_flag[SDP_DEBUG_ERRORS]) {
                SDP_ERROR("%s group id number %u is outside the "
                          "range of actual ids set",
                          sdp_p->debug_str, id_num);
            }
            sdp_p->conf_p->num_invalid_param++;
            return (NULL);
        }
    }
    return (attr_p->attr.group_data.group_id_arr[id_num-1]);
}

/* Function:    sdp_set_group_id
 * Description: Sets a particular group id in the
 *              a=group:<val> <id> <id> ... line.
 *              Also sets the number of group ids if this is the
 *              maximum id number set so far.
 * Parameters:  sdp_ptr     The SDP handle returned by sdp_init_description.
 *              level       SDP_SESSION_LEVEL
 *              cap_num     The capability number associated with the
 *                          attribute if any.  If none, should be zero.
 *              inst_num    The attribute instance number to check.
 *              id_num      Number of the id to set. The range is (1 - 
 *                          SDP_MAX_GROUP_STREAM_ID)
 * Returns:     SDP_SUCCESS or SDP_INVALID_PARAMETER/SDP_INVALID_SDP_PTR
*/
sdp_result_e sdp_set_group_id (void *sdp_ptr, u16 level, 
                               u8 cap_num, u16 inst_num,
                               const char* group_id, u16 id_num)
{
    sdp_t       *sdp_p = (sdp_t *)sdp_ptr;
    sdp_attr_t  *attr_p;
	...
	if ((id_num < 1) || (id_num > SDP_MAX_GROUP_STREAM_ID)) {

            if (sdp_p->debug_flag[SDP_DEBUG_ERRORS]) {
                SDP_ERROR("%s group id number %u is out of range",
                          sdp_p->debug_str, id_num);
            }
            sdp_p->conf_p->num_invalid_param++;
            return (SDP_INVALID_PARAMETER);
        ...
        /*
         * Set the number of group ids if this is the max id 
         * set so far.
         */
        if (id_num > attr_p->attr.group_data.num_group_id) {
            attr_p->attr.group_data.num_group_id = id_num;
        }

        if (attr_p->attr.group_data.group_id_arr[id_num-1]) {
            SDP_FREE(attr_p->attr.group_data.group_id_arr[id_num-1]);
        }
        attr_p->attr.group_data.group_id_arr[id_num-1] = strdup(group_id);
...
/* Function:    sdp_get_rtcp_port
 * Description: Gets the rtcp port
 ...
 * Returns:     rtcp port value
 */
u32
sdp_get_rtcp_port (void *sdp_ptr, u16 level, u8 cap_num, u16 inst_num)
{
    sdp_t      *sdp_p = (sdp_t *)sdp_ptr;
    sdp_attr_t *attr_p;
    ...
    attr_p = sdp_find_attr(sdp_p, level, cap_num, 
                           SDP_ATTR_RTCP, inst_num);
    if (attr_p == NULL) {
        if (sdp_p->debug_flag[SDP_DEBUG_ERRORS]) {
            SDP_ERROR("%s rtcp attribute, level %u, "
    ...
    return (attr_p->attr.rtcp.port);
}

/* Function:    sdp_get_rtcp_conn_address
 * Description: Gets the connection address parameters
 ...
 */
sdp_result_e
sdp_get_rtcp_conn_address (void *sdp_ptr, u16 level, u8 cap_num,
                           ...
                           char *dest_addr)
{
    ...
    if (attr_p == NULL) {
        if (sdp_p->debug_flag[SDP_DEBUG_ERRORS]) {
            SDP_ERROR("%s rtcp attribute, level %u instance %u "
                      "not found", sdp_p->debug_str, level, inst_num);
        }
        sdp_p->conf_p->num_invalid_param++;
        return (SDP_INVALID_PARAMETER);
    ...
    if (nettype) {
        *nettype = attr_p->attr.rtcp.nettype;
    }

    if (addrtype) {
        *addrtype = attr_p->attr.rtcp.addrtype;
    }

    sstrncpy(dest_addr, attr_p->attr.rtcp.conn_addr, SDP_MAX_STRING_LEN+1);
...
/* Function:    sdp_get_rtcp_fb__nack_param
 * Description: Returns the parameters from the "a=rtcp-fb:<payload> nack"  line.  
 * Parameters:  sdp_ptr     The SDP handle returned by sdp_init_description.
 *              level       SDP_SESSION_LEVEL
 * Returns:     
 */
sdp_result_e sdp_get_rtcp_fb_nack_param (void *sdp_ptr, u16 level,
                                         u8 cap_num, u16 inst_num,
                                         u16 *pt_num, char *param)
{
    sdp_t               *sdp_p = (sdp_t *)sdp_ptr;
    sdp_attr_t          *attr_p;
    ...
    attr_p = sdp_find_attr(sdp_p, level, cap_num, 
                           SDP_ATTR_RTCP_FB, inst_num);
    if (attr_p == NULL) {
        if (sdp_p->debug_flag[SDP_DEBUG_ERRORS]) {
            SDP_ERROR("%s a=rtcp-fb attribute, level %u instance %u "
                      "not found.", sdp_p->debug_str, level, inst_num);
        }
        sdp_p->conf_p->num_invalid_param++;
        return (0);
        ...
        if (sdp_p->debug_flag[SDP_DEBUG_TRACE]) {
            SDP_PRINT("%s RTCP feedback attr - payload is :%d ", 
                      sdp_p->debug_str, 
                      attr_p->attr.rtcp_fb.payload_num);
        }

        *pt_num = attr_p->attr.rtcp_fb.payload_num;
        sstrncpy(param, attr_p->attr.rtcp_fb.rtcp_fb_nack_param,
                 SDP_MAX_STRING_LEN+1);
    }
