/*
 *------------------------------------------------------------------
 * sdp_main.c  -- General routines for the generic SDP parser.
 *
...
const sdp_attrarray_t sdp_attr[SDP_MAX_ATTR_TYPES] =
{
    ...
    {"source-filter", sizeof("source-filter"), 
      sdp_parse_attr_source_filter, sdp_build_attr_source_filter},
    ...
    {"cat", sizeof("cat"), 
     sdp_parse_attr_simple_string, sdp_build_attr_simple_string },
    {"label", sizeof("label"), 
     sdp_parse_attr_string, sdp_build_attr_simple_string },
    {"rtcp-fb", sizeof("rtcp-fb"),
     sdp_parse_attr_rtcp_fb, sdp_build_attr_rtcp_fb },
    {"rtcp-xr", sizeof("rtcp-xr"),
     sdp_parse_attr_rtcp_xr, sdp_build_attr_rtcp_xr },
    {"rtcp-rsize", sizeof("rtcp-rsize"),
     sdp_parse_attr_rtcp_rsize, sdp_build_attr_rtcp_rsize }
};
...
/* Note: These *must* be in the same order as the enum type. */
const sdp_namearray_t sdp_qos_direction[SDP_MAX_QOS_DIRECTIONS] =
{
    {"send",              sizeof("send")},
    {"recv",              sizeof("recv")},
    {"sendrecv",          sizeof("sendrecv")},
...
const sdp_namearray_t sdp_fmtp_codec_param[SDP_MAX_FMTP_PARAM] =
{
    ...
    {"rtcp-per-rcvr-bw",         sizeof("rtcp-per-rcvr-bw")},
} ;
...
/* Note: These *must* be in the same order as the enum type. */
const sdp_namearray_t sdp_fmtp_rtp_retrans_param
...
const sdp_namearray_t sdp_rtcp_xr_param[SDP_MAX_RTCP_XR_VAL] =
{
    {"pkt-loss-rle",        sizeof("pkt-loss-rle")},
    {"pkt-dup-rle",         sizeof("pkt-dup-rle")},
    {"pkt-rcpt-times",      sizeof("pkt-rcpt-times")},
    {"rcvr-rtt",            sizeof("rcvr-rtt")},
    {"stat-summary",        sizeof("stat-summary")},
    {"voip-metrics",        sizeof("voip-metrics")},
    {"post-repair-loss-rle", sizeof("post-repair-loss-rle")},
    {"multicast-acq",       sizeof("multicast-acq")},
    {"vqe-diagnostic-counters", sizeof("vqe-diagnostic-counters")},
};
...
{
    {"FID",                 sizeof("FID")}, /* Flow Identification */
    {"LS",                  sizeof("LS")},  /* Lip Synchronization */
    {"FEC",                 sizeof("FEC")}  /* Forward Error Correction */
};
...
const char* sdp_result_name[SDP_MAX_RC] = 
    {"SDP_SUCCESS", 
     ...
     "UNRECOGNIZED_TOKEN",
     "NULL_BUF_PTR",
     "POTENTIAL_SDP_OVERFLOW",
     "SDP_VERSION_LINE_NOT_FIRST",
     "FIELD_VALUE_OVERFLOW",
     "INVALID_BOOLEAN",
     "INVALID_RTCP_PER_RCVR_BW_LINE",
     "INVALID_FMTP_RTX_LINE",
     "INVALID_RTPMAP_LINE",
     "INVALID_CLOCK_IN_RTPMAP_LINE",
     "INVALID_GROUP_LINE",
     "INVALID_RTCP_LINE",
     "INVALID_SRC_FILTER_LINE",
     "INVALID_RTCP_UNICAST_LINE",
     "INVALID_RTCP_FB_LINE",
     "INVALID_RTCP_XR_LINE",
    };
...
const char *sdp_get_bw_modifier_name (sdp_bw_modifier_e bw_modifier_type)
{
    if (bw_modifier_type == SDP_BW_MODIFIER_UNSUPPORTED) {
        return (SDP_UNSUPPORTED);
    } else if (bw_modifier_type >= SDP_MAX_BW_MODIFIER_VAL) {
...
sdp_result_e sdp_validate_sdp (sdp_t *sdp_p)
{
    ...
    if (sdp_connection_valid((void *)sdp_p, SDP_SESSION_LEVEL) == FALSE) {
        ...
        for (i=1; i <= num_media_levels; i++) {
            if (sdp_connection_valid((void *)sdp_p, i) == FALSE) {
...
sdp_result_e sdp_parse (void *sdp_ptr, char **bufp, u16 len)
{
	 ...
	 */
	  ...
	  (sdp_p->debug_flag[SDP_DEBUG_TRACE])) {
	    ...
	    SDP_PRINT("%*s", (int)(intptr_t)(line_end - ptr), ptr);
        ...
        if (first_line == TRUE) {
            if (i != SDP_TOKEN_V) {
                if (sdp_p->conf_p->version_reqd == TRUE) {
                    ...
                    sdp_p->conf_p->num_invalid_token_order++;
                    return (SDP_VERSION_LINE_NOT_FIRST);
                } else {
            ...
            if (i < last_token) {
                sdp_p->conf_p->num_invalid_token_order++;
                if (sdp_p->debug_flag[SDP_DEBUG_ERRORS]) {
                    SDP_ERROR("%s Warning: Invalid token ordering detected, "
                              "token %s found after token %s",
                              sdp_p->debug_str, sdp_token[i].name, 
                              sdp_token[last_token].name);
                }
...
sdp_result_e sdp_build (void *sdp_ptr, char **bufp, u16 len)
{
    ...
    for (i=0; ((i < SDP_TOKEN_M) && (result == SDP_SUCCESS)); i++) {
        result = sdp_token[i].build_func(sdp_p, SDP_SESSION_LEVEL, &ptr, len);
        len = endbuf_p - ptr;
    }
    /* If the session level was ok, build the media lines. */
    if (result == SDP_SUCCESS) {
        for (i=1; i <= sdp_p->mca_count; i++) {
            result = sdp_token[SDP_TOKEN_M].build_func(sdp_p, i, &ptr, len);
            len = endbuf_p - ptr;
            for (j=SDP_TOKEN_I;
                 ((j < SDP_TOKEN_M) && (result == SDP_SUCCESS)); j++) {
                ...
                result = sdp_token[j].build_func(sdp_p, i, &ptr, len);
                len = endbuf_p - ptr;
            }
        }
    }
...
void *sdp_copy (void *sdp_ptr)
{
    ...
    sstrncpy(new_sdp_p->sessname, orig_sdp_p->sessname,
             SDP_MAX_LINE_LEN+1);
    sstrncpy(new_sdp_p->sessinfo, orig_sdp_p->sessinfo,
             SDP_MAX_LINE_LEN+1);
    ...
    while (orig_mca_p != NULL) {
        ...
        sdp_copy_all_attrs(orig_sdp_p, new_sdp_p, cur_level, cur_level);
