/*
 *------------------------------------------------------------------
 * sdp_token.c  -- Routines for parsing and building all token lines
 *                 except attributes.
 *
...
sdp_result_e sdp_parse_owner (sdp_t *sdp_p, u16 level, const char *ptr)
{
    ...
    if (result == SDP_SUCCESS) {
         ...
         * a string.
         */
        if (sdp_check_for_non_numeric(sdp_p->owner_sessid)) {
            if (sdp_p->debug_flag[SDP_DEBUG_ERRORS]) {
    ...
    if (result == SDP_SUCCESS) {
        if (strlen(sdp_p->owner_version) > 20) {
            sdp_p->conf_p->num_invalid_param++;
            return (SDP_FIELD_VALUE_OVERFLOW);
        }
                
        /* Make sure the version is numeric, even though we store it as
         * a string.
         */
        if (sdp_check_for_non_numeric(sdp_p->owner_version)) {
            if (sdp_p->debug_flag[SDP_DEBUG_ERRORS]) {
...
sdp_result_e sdp_build_owner (sdp_t *sdp_p, u16 level, char **ptr, u16 len)
{
    ...
    if ((sdp_p->owner_name[0] == '\0') ||
        ...
        if((sdp_p->owner_network_type == SDP_NT_ATM) &&
          ...
          *ptr += snprintf(*ptr, len, "o=%s %s %s %s - -\r\n", 
    ...
    *ptr += snprintf(*ptr, len, "o=%s %s %s %s %s %s\r\n", 
                    sdp_p->owner_name, sdp_p->owner_sessid,
                    sdp_p->owner_version, 
                    sdp_get_network_name(sdp_p->owner_network_type),
                    sdp_get_address_name(sdp_p->owner_addr_type),
...
sdp_result_e sdp_parse_sessname (sdp_t *sdp_p, u16 level, const char *ptr)
{
    ...
    if ((endptr - ptr) >= SDP_MAX_STRING_LEN) {
        sdp_p->conf_p->num_invalid_token_order++;
        return (SDP_FIELD_VALUE_OVERFLOW);
    }

    str_len = SDP_MIN(endptr - ptr, SDP_MAX_STRING_LEN);
...
sdp_result_e sdp_parse_sessinfo (sdp_t *sdp_p, u16 level, const char *ptr)
{
    ...
    /* Save only the info from session-level */
    if (level == SDP_SESSION_LEVEL && sdp_p->sessinfo_found) {
        int str_len = SDP_MIN(endptr - ptr, SDP_MAX_STRING_LEN);
        sstrncpy(sdp_p->sessinfo, ptr, str_len+1);
    }
...
/*
 * sdp_parse_mcast_connection_addr
 *
 * Parses connection data for multicast connection address
 *
 * c=<base multicast address>/<ttl>/<number of addresses>
 *
 * Inputs:
 *   sdp_p  - pointer to sdp handle
 *   level  - level to check for the attribute.
 */
sdp_result_e sdp_parse_mcast_connection_addr (sdp_t *sdp_p, u16 level)
{
    sdp_result_e  result;
    sdp_conn_t   *conn_p;
    sdp_mca_t    *mca_p;
    ...
    *slash_ptr = 0;
    ...
    /* get TTL */
    conn_p->ttl = sdp_getnextnumtok(ttl_ptr, &ttl_ptr,  "/", &result);
    if (result != SDP_SUCCESS) {
        if (sdp_p->debug_flag[SDP_DEBUG_ERRORS]) {
            SDP_ERROR("%s No ttl value specified for this multicast addr",
                                 sdp_p->debug_str);
        }
        sdp_p->conf_p->num_invalid_param++;
        return (SDP_INVALID_PARAMETER);
    ...
    if (slash_ptr[0] != '\0') {
        ...
        if (slash_ptr[0] == '\0') {
            if (sdp_p->debug_flag[SDP_DEBUG_ERRORS]) {
                SDP_ERROR("%s Invalid multicast connection address format",
                      sdp_p->debug_str);
            }
            sdp_p->conf_p->num_invalid_param++;
            return (SDP_INVALID_PARAMETER);
        ...
        if ((conn_p->num_of_addresses > 1) && (level == SDP_SESSION_LEVEL)) {
            if (sdp_p->debug_flag[SDP_DEBUG_ERRORS]) {
                SDP_ERROR("%s For multicast connection address number of"
                    " addresses can only be specified at the media level", 
                      sdp_p->debug_str);
            }
            sdp_p->conf_p->num_invalid_param++;
            return (SDP_INVALID_PARAMETER);
        ...
        /* default value for number of addresses is 1 */
        conn_p->num_of_addresses = 1;
    }
...
sdp_result_e sdp_parse_connection (sdp_t *sdp_p, u16 level, const char *ptr)
{
    ...
    sstrncpy (mcast_str, conn_p->conn_addr, 4);
    ...
    if (conn_p->addrtype != SDP_AT_EPN) {
        ...
        if (slash_ptr[0] != '\0') {
            if (conn_p->is_multicast) {
                result = sdp_parse_mcast_connection_addr(sdp_p, level);
                if (result != SDP_SUCCESS) {
                    return (result);
                }
            } else {
                if (sdp_p->debug_flag[SDP_DEBUG_ERRORS]) {
                    SDP_ERROR("%s Unsupported connection address format for "
                          "c=, parse failed.", sdp_p->debug_str);
                }
                sdp_p->conf_p->num_invalid_param++;
                return (SDP_INVALID_PARAMETER);
            }
        } else if (conn_p->is_multicast) {
            if (sdp_p->debug_flag[SDP_DEBUG_ERRORS]) {
                SDP_ERROR("%s Invalid multicast connection address format"
                          "c=, parse failed.", sdp_p->debug_str);
            }
                                   ...
                                   u16 len)
{
    ...
    if (conn_p->is_multicast) {
        if (conn_p->num_of_addresses > 1) {
            *ptr += snprintf(*ptr, len, "c=%s %s %s/%d/%d\r\n", 
                             sdp_get_network_name(conn_p->nettype),
                             sdp_get_address_name(conn_p->addrtype),
                             conn_p->conn_addr, conn_p->ttl, 
                             conn_p->num_of_addresses);
        } else {
            *ptr += snprintf(*ptr, len, "c=%s %s %s/%d\r\n", 
                             sdp_get_network_name(conn_p->nettype),
                             sdp_get_address_name(conn_p->addrtype),
                             conn_p->conn_addr, conn_p->ttl);
        }
    } else {
        *ptr += snprintf(*ptr, len, "c=%s %s %s\r\n", 
                         sdp_get_network_name(conn_p->nettype),
                         sdp_get_address_name(conn_p->addrtype),
                         conn_p->conn_addr);
    }
...
sdp_result_e sdp_parse_bandwidth (sdp_t *sdp_p, u16 level, const char *ptr)
{
    ...
    /* Find the bw type (AS, RS, RR, CT or TIAS) */
    ptr = sdp_getnextstrtok(ptr, tmp, ":", &result);
    if (result != SDP_SUCCESS) {
        if (sdp_p->debug_flag[SDP_DEBUG_ERRORS]) {
    ...
    for (i=0; i < SDP_MAX_BW_MODIFIER_VAL; i++) {
        if (strncasecmp(tmp, sdp_bw_modifier_val[i].name,
...
sdp_result_e sdp_parse_timespec (sdp_t *sdp_p, u16 level, const char *ptr)
{
    ...
    if (result == SDP_SUCCESS) {
         ...
         * a string.
         */
        if (sdp_check_for_non_numeric(timespec_p->start_time)) {
            if (sdp_p->debug_flag[SDP_DEBUG_ERRORS]) {
    ...
    if (result == SDP_SUCCESS) {
        /* Make sure the stop_time is numeric, even though we store it as
         * a string.
         */
        if (sdp_check_for_non_numeric(timespec_p->stop_time)) {
            if (sdp_p->debug_flag[SDP_DEBUG_ERRORS]) {
...
sdp_result_e sdp_parse_encryption (sdp_t *sdp_p, u16 level, const char *ptr)
{
    ...
    ptr = sdp_getnextstrtok(ptr, tmp, ":", &result);
    if ((result != SDP_SUCCESS) || (ptr == NULL)) {
        if (sdp_p->debug_flag[SDP_DEBUG_ERRORS]) {
    ...
    for (i=0; i < SDP_MAX_ENCRYPT_TYPES; i++) {
        if (strncasecmp(tmp, sdp_encrypt[i].name,
...
sdp_result_e sdp_parse_media (sdp_t *sdp_p, u16 level, const char *ptr)
{
    ...
    for (i=0; i < SDP_MAX_MEDIA_TYPES; i++) {
        if (strncasecmp(tmp, sdp_media[i].name,
    ...
    case 1:
	    ...
	    (mca_p->transport == SDP_TRANSPORT_RTPAVPF) ||
    ...
    if (sdp_p->mca_p == NULL) {
        ...
        for (next_mca_p = sdp_p->mca_p; next_mca_p->next_p != NULL;
             next_mca_p = next_mca_p->next_p);  /* Empty for */
...
sdp_result_e sdp_build_media (sdp_t *sdp_p, u16 level, char **ptr, u16 len)
{
    ...
    *ptr += snprintf(*ptr, len, "m=%s ", sdp_get_media_name(mca_p->media));
    len = endbuf_p - *ptr;
    ...
    if (mca_p->port_format == SDP_PORT_NUM_ONLY) {
        ...
        *ptr += snprintf(*ptr, len, "%u/%u/%u/%u ", (u16)mca_p->port, 
                        (u16)mca_p->vpi, (u16)mca_p->vci, (u16)mca_p->cid);
    }
    /* Recalculate remaining buffer after printing port info. */
    len = endbuf_p - *ptr;
    ...
    *ptr += snprintf(*ptr, len, "%s", 
    ...
    for (i=0; i < mca_p->num_payloads; i++) {
        if (mca_p->payload_indicator[i] == SDP_PAYLOAD_ENUM) {
            ...
            *ptr += snprintf(*ptr, len, " %u", mca_p->payload_type[i]);
        }
        len = endbuf_p - *ptr;
    }
    *ptr += snprintf(*ptr, len, "\r\n");
...
void sdp_parse_payload_types (sdp_t *sdp_p, sdp_mca_t *mca_p, const char *ptr)
{
    ...
    for (num_payloads = 0; (num_payloads < SDP_MAX_PAYLOAD_TYPES); ) {
        ...
        mca_p->payload_type[num_payloads] = sdp_getnextnumtok(tmp, 
                                                        (const char **)&tmp2,
                                                        " \t", &result);
        if (result == SDP_SUCCESS) {
        ...
        for (i=0; i < SDP_MAX_STRING_PAYLOAD_TYPES; i++) {
            if (strncasecmp(tmp, sdp_payload[i].name,
                                                 ...
                                                 const char *ptr)
{
    ...
    while (TRUE) {
        ...
        profile_p->payload_type[prof][payload] = sdp_getnextnumtok(tmp, 
                                                         (const char **)&tmp2,
                                                         " \t", &result);
        if (result == SDP_SUCCESS) {
